# AnnotatedScrollBar Specification

## Purpose

规范 scrolling 模块中的 Fluent `AnnotatedScrollBar` 控件：以 WinUI 风格显示带标签和详情提示的竖向滚动控制器，支持标签 offset 映射与碰撞隐藏、detail label provider、ScrollView 联动，以及对应的主题与测试要求。
