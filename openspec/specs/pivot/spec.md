...
The system SHALL provide `view::navigation::Pivot` as a lightweight WinUI-style Fluent page-category switcher for Qt Widgets. Pivot owns its item header strip, selection indicator, overflow behavior, pointer and keyboard interaction, accessibility text, and stacked page content management while leaving section labels such as `EMAIL` anchored externally by callers.
