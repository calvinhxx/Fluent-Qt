# selector-bar Specification

## Purpose

The system SHALL provide `view::navigation::SelectorBar` as a lightweight WinUI-style Fluent selector for Qt Widgets. SelectorBar owns an ordered horizontal item row, selection state, selected indicator, hidden and disabled item behavior, overflow, pointer and keyboard interaction, accessibility text, and theme-aware visuals while leaving pages, routes, frames, content widgets, and external presenters under caller ownership.
