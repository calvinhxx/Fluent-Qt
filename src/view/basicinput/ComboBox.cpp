...
void ComboBox::ComboBoxPopup::showForComboBox() {
    ...
    const QSize totalSize = ::view::overlay::outerSizeForVisibleCard(QSize(cardW, cardH), sSize);

    setFixedSize(totalSize);
    setAnchorOffset(m_comboBox->m_popupOffset);
    setAnchor(m_comboBox);

    const QRect cardRect = ::view::overlay::visibleCardRect(rect(), sSize);
...
QPoint ComboBox::ComboBoxPopup::computePosition() const {
    ...
    const QSize cardSize = ::view::overlay::visibleCardSize(size(), shadow);
    ...
    if (clampToWindow()) {
        cardTopLeft = ::view::overlay::clampCardTopLeft(cardTopLeft, cardSize, top->rect(), kPopupWindowMargin);
    }

    return ::view::overlay::outerTopLeftForVisibleCard(cardTopLeft, shadow);
}
...
bool ComboBox::ComboBoxPopup::eventFilter(QObject* watched, QEvent* event) {
    if (event && event->type() == QEvent::MouseButtonPress && m_comboBox) {
        ...
        const bool pressInsidePopup = ::view::overlay::visibleCardContains(rect(), mapFromGlobal(fluentMouseGlobalPos(mouseEvent)), kPopupShadowMargin);
