...
QPoint Flyout::clampCardPos(const QPoint& cardTopLeft) const {
    ...
    const QSize cardSize = ::view::overlay::visibleCardSize(size());

    const int margin = 4;  // 与窗口边缘留点呼吸空间
    return ::view::overlay::clampCardTopLeft(cardTopLeft, cardSize, top->rect(), margin);
}
...
Flyout::Placement Flyout::resolveAutoPlacement() const {
    ...
    const int cardH = ::view::overlay::visibleCardSize(size()).height();
...
QPoint Flyout::computePosition() const {
    ...
    return ::view::overlay::outerTopLeftForVisibleCard(card);
}
