...
#include "view/foundation/overlay/OverlayLightDismiss.h"
...
Popup::Popup(QWidget* parent) : QWidget(parent) {
    ...
    const int shadowMargin = ::view::overlay::defaultShadowMargin();
    setContentsMargins(shadowMargin, shadowMargin, shadowMargin, shadowMargin);
    resize(::view::overlay::outerSizeForVisibleCard(QSize(320, 160), shadowMargin));
...
void Popup::open() {
    ...
    if (m_positionSet)
        move(::view::overlay::outerTopLeftForVisibleCard(m_targetPos));
    else
...
void Popup::ensureScrim() {
    ...
    if (!m_scrim)
        m_scrim = new ::view::overlay::OverlayScrim(top, QStringLiteral("PopupScrim"));
...
bool Popup::eventFilter(QObject* watched, QEvent* event) {
    ...
    if (::view::overlay::isEscapeKeyPress(event) &&
        ::view::overlay::allowsImplicitClose(noAutoClose, m_closePolicy & CloseOnEscape)) {
        close();
        event->accept();
        return true;
    }

    if (!::view::overlay::allowsImplicitClose(noAutoClose, m_closePolicy & CloseOnPressOutside)) return false;
    ...
    if (::view::overlay::visibleCardContains(rect(), local)) return false;
...
void Popup::paintEvent(QPaintEvent*) {
    ...
    const QRect contentRect = ::view::overlay::visibleCardRect(rect());
