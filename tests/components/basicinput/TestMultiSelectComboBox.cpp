...
class MultiSelectComboBoxTestWindow final : public QWidget, public fluent::FluentElement {
public:
    ...
    void onThemeUpdated() override
    {
        ...
        setStyleSheet(QStringLiteral("background-color: %1;").arg(colors.bgCanvas.name()));
    }
};
...
fluent::dialogs_flyouts::Flyout* openPopup(MultiSelectComboBox& box, QWidget& window)
{
    window.show();
    box.show();
...
{
    const QRect visible = fluent::overlay::visibleCardRect(QRect(QPoint(), popup.size()));
    return QRect(popup.mapTo(&window, visible.topLeft()), visible.size());
}

bool sendWheel(QWidget* target, QPoint pixelDelta, QPoint angleDelta, Qt::ScrollPhase phase)
{
    const QPoint local = target->rect().center();
    QWheelEvent event(QPointF(local), QPointF(target->mapToGlobal(local)), pixelDelta, angleDelta,
...
TEST_F(MultiSelectComboBoxTest, EmptyModelDisablesSelectAllHeader)
{
    ...
    auto* selectAll = popup->findChild<CheckBox*>(QStringLiteral("MultiSelectComboBox.SelectAll"));
...
{
    QStringListModel model({QStringLiteral("North America"), QStringLiteral("Europe"),
...
{
    QStringListModel model({QString::fromUtf8("الشرق الأوسط"), QString::fromUtf8("شمال أفريقيا")});
    MultiSelectComboBox box(window);
...
TEST_F(MultiSelectComboBoxTest, RootSelectionPreservesOtherRoots)
{
    ...
    box.clearSelection();
    EXPECT_TRUE(box.selectedRows().isEmpty());
    EXPECT_TRUE(box.selectionModel()->isSelected(model.index(0, 0, secondRootIndex)));
}

TEST_F(MultiSelectComboBoxTest, DestroyingModelRestoresEmptyInternalState)
{
    MultiSelectComboBox box(window);
    auto* model = new QStringListModel({QStringLiteral("Alpha"), QStringLiteral("Beta")});
    box.setModel(model);
    box.setSelectedRows({1});
...
{
    auto* model = new QStringListModel({QStringLiteral("Alpha"), QStringLiteral("Beta")}, window);
...
TEST_F(MultiSelectComboBoxTest, PopupUsesRoomierSideBeforeClipping)
{
    ...
    EXPECT_EQ(list->height(),
              box.maximumVisibleItems() * (Spacing::ControlHeight::Standard + Spacing::XSmall));
}
...
TEST_F(MultiSelectComboBoxTest, ConstrainedPopupAcceptsWheelAndTrackpadScroll)
{
    ...
    EXPECT_TRUE(sendWheel(list->viewport(), QPoint(0, -24), QPoint(), Qt::ScrollUpdate));
    QTest::qWait(20);
...
TEST_F(MultiSelectComboBoxTest, PopupContentStaysInsideVisibleCard)
{
    ...
    auto* search = window->findChild<fluent::textfields::LineEdit*>(
        QStringLiteral("MultiSelectComboBox.Search"));
    auto* selectAll = window->findChild<CheckBox*>(QStringLiteral("MultiSelectComboBox.SelectAll"));
    auto* list = window->findChild<fluent::collections::ListView*>(
        QStringLiteral("MultiSelectComboBox.ListView"));
    ASSERT_NE(search, nullptr);
    ASSERT_NE(selectAll, nullptr);
    ...
    EXPECT_EQ(selectAll->height(), Spacing::ControlHeight::Standard + Spacing::XSmall);
    ...
    EXPECT_TRUE(list->viewport()->property("fluentPreserveParentSurface").toBool());
    ...
    const QRect restingRow = static_cast<QListView*>(list)->visualRect(list->model()->index(1, 0));
    ASSERT_TRUE(restingRow.isValid());
    EXPECT_EQ(restingRow.height(), Spacing::ControlHeight::Standard + Spacing::XSmall);
    const QPoint surfaceSample =
        list->viewport()->mapTo(window, QPoint(restingRow.center().x(), restingRow.top() + 1));
    ...
    const QRect card = fluent::overlay::visibleCardRect(QRect(QPoint(), popup->size()));
    const QRect contentBounds =
        card.adjusted(Spacing::XSmall, Spacing::XSmall, -Spacing::XSmall, -Spacing::XSmall);
    for (QWidget* child : {static_cast<QWidget*>(search), static_cast<QWidget*>(selectAll),
        ...
        EXPECT_TRUE(contentBounds.contains(child->geometry()))
            ...
            << child->geometry().width() << 'x' << child->geometry().height();
    }
}
...
TEST_F(MultiSelectComboBoxTest, SelectAllHeaderTargetsFilteredRows)
{
    ...
    QAccessibleInterface* selectAllInterface = QAccessible::queryAccessibleInterface(selectAll);
    ASSERT_NE(selectAllInterface, nullptr);
    EXPECT_TRUE(selectAllInterface->state().checkStateMixed);
    QAccessibleActionInterface* selectAllActions = selectAllInterface->actionInterface();
    ASSERT_NE(selectAllActions, nullptr);
    EXPECT_TRUE(
        selectAllActions->actionNames().contains(QAccessibleActionInterface::pressAction()));
...
TEST_F(MultiSelectComboBoxTest, PointerCannotSelectDisabledPopupRow)
{
    ...
    const QRect target = static_cast<QListView*>(list)->visualRect(disabledIndex);
...
TEST_F(MultiSelectComboBoxTest, PopupFocusVisualTracksInputModality)
{
    ...
    const QRect firstRow = static_cast<QListView*>(list)->visualRect(model.index(0, 0));
    ASSERT_TRUE(firstRow.isValid());
    QTest::mouseClick(list->viewport(), Qt::LeftButton, Qt::NoModifier, firstRow.center());
    QApplication::processEvents();
    EXPECT_FALSE(list->property("fluentKeyboardFocusVisible").toBool());
...
TEST_F(MultiSelectComboBoxTest, AccessibleRootExposesValueAndPopupAction)
{
    ...
    EXPECT_EQ(interface->text(QAccessible::Name), QStringLiteral("Included regions"));
    EXPECT_EQ(interface->text(QAccessible::Value), QString::fromUtf8("Alpha、Gamma"));
    ...
    QWidget* list = window->findChild<QWidget*>(QStringLiteral("MultiSelectComboBox.ListView"));
    QWidget* trigger = window->findChild<QWidget*>(QStringLiteral("MultiSelectComboBox.Trigger"));
    QWidget* search = window->findChild<QWidget*>(QStringLiteral("MultiSelectComboBox.Search"));
    ASSERT_NE(list, nullptr);
    ASSERT_NE(trigger, nullptr);
    ASSERT_NE(search, nullptr);
    for (QWidget* namedWidget : {trigger, search, list}) {
        QAccessibleInterface* namedInterface = QAccessible::queryAccessibleInterface(namedWidget);
    ...
    for (const auto& relation : interface->relations(QAccessible::Controller)) {
        controlsList = controlsList || (relation.first == listInterface &&
    ...
    EXPECT_TRUE(betaActions->actionNames().contains(QAccessibleActionInterface::pressAction()));
    betaActions->doAction(QAccessibleActionInterface::pressAction());
    QApplication::processEvents();
    EXPECT_TRUE(beta->state().selected);
    EXPECT_EQ(interface->text(QAccessible::Value), QString::fromUtf8("Alpha、Beta、Gamma"));
    EXPECT_TRUE(box.isOpen());

    QAccessibleInterface* otherInterface = QAccessible::queryAccessibleInterface(&other);
...
TEST_F(MultiSelectComboBoxTest, VisualCheck)
{
    ...
    auto* title = new fluent::textfields::Label(QStringLiteral("MultiSelectComboBox"), window);
    ...
    auto* basicLabel = new fluent::textfields::Label(QStringLiteral("Regions"), window);
    basicLabel->setFluentTypography(Typography::FontRole::BodyStrong);
    basicLabel->setTextColorRole(fluent::textfields::Label::TextColorRole::Primary);
    ...
    auto* basicModel = new QStringListModel(
        {QStringLiteral("North America"), QStringLiteral("Europe"), QStringLiteral("Asia Pacific"),
         QStringLiteral("Latin America"), QStringLiteral("Middle East and Africa"),
         QString::fromUtf8("中国大陆"), QString::fromUtf8("日本")},
        window);
    ...
    auto* narrowLabel = new fluent::textfields::Label(QStringLiteral("Compact summary"), window);
    narrowLabel->setFluentTypography(Typography::FontRole::BodyStrong);
    narrowLabel->setTextColorRole(fluent::textfields::Label::TextColorRole::Primary);
    ...
    auto* rtlLabel =
        new fluent::textfields::Label(QStringLiteral("Arabic (right-to-left)"), window);
    ...
    auto* rtlModel =
        new QStringListModel({QString::fromUtf8("الشرق الأوسط"), QString::fromUtf8("شمال أفريقيا"),
    ...
    auto* disabledLabel = new fluent::textfields::Label(QStringLiteral("Disabled"), window);
    disabledLabel->setFluentTypography(Typography::FontRole::BodyStrong);
    disabledLabel->setTextColorRole(fluent::textfields::Label::TextColorRole::Primary);
    ...
    auto* themeButton = new fluent::basicinput::Button(QStringLiteral("Dark theme"), window);
    ...
    QObject::connect(themeButton, &fluent::basicinput::Button::clicked, [themeButton]() {
        fluent::FluentElement::setTheme(fluent::FluentElement::currentTheme() ==
                                                fluent::FluentElement::Light
                                            ? fluent::FluentElement::Dark
                                            : fluent::FluentElement::Light);
        themeButton->setText(fluent::FluentElement::currentTheme() == fluent::FluentElement::Light
