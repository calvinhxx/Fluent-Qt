...
{
    return qobject_cast<QGraphicsOpacityEffect*>(tooltip.graphicsEffect());
}
...
protected:
    void SetUp() override
    {
        ...
        container->resize(600,
                          600); // Should differ from scrollarea size to show scrolling if needed
...
TEST_F(ToolTipTest, InitialState)
{
    ...
    EXPECT_DOUBLE_EQ(opacityEffectFor(tooltip)->opacity(), 0.0);
    auto* textBlock = tooltip.findChild<Label*>();
    ASSERT_NE(textBlock, nullptr);
    EXPECT_TRUE(textBlock->styleSheet().contains(QStringLiteral("color:")));
}
...
TEST_F(ToolTipTest, AttachedToolTipUsesFluentBubbleAboveTarget)
{
    ...
    QHelpEvent help(QEvent::ToolTip, target->rect().center(),
                    target->mapToGlobal(target->rect().center()));
    QApplication::sendEvent(target, &help);
    QApplication::processEvents();

    ASSERT_TRUE(tip->isVisible());
    const QRect bubble = tip->geometry().adjusted(tip->shadowMargin(), tip->shadowMargin(),
                                                  -tip->shadowMargin(), -tip->shadowMargin());
...
TEST_F(ToolTipTest, AnimationEnabledDefaultAndDisabledBehavior)
{
    ...
    ASSERT_NE(opacityEffectFor(tooltip), nullptr);
    EXPECT_NEAR(opacityEffectFor(tooltip)->opacity(), 1.0, 0.001);
    EXPECT_DOUBLE_EQ(tooltip.windowOpacity(), 1.0);
...
TEST_F(ToolTipTest, MotionPolicyDisabledSettlesShowAndHideSynchronously)
{
    fluent::MotionPolicy::instance().setMode(fluent::MotionPolicy::Mode::Disabled);
    ToolTip tooltip;
    tooltip.setText(QStringLiteral("No motion"));
