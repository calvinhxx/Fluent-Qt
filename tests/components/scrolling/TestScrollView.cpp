...
TEST_F(ScrollViewTest, ScrollChainingModeControlsBoundaryWheel) {
    ...
    // Default (chaining off): boundary wheel input is consumed in place.
    // zh_CN: 默认（链式关闭）：边缘滚轮被就地消费。
    ...
    // Chaining on: the same boundary wheel stays ignored so it can bubble to an
    // enclosing scroller.
    // zh_CN: 链式开启：同样的边缘滚轮保持 ignore，可冒泡给外层滚动容器。
