...
qreal FluentTreeItemDelegate::accentProgress(const QModelIndex& index) const {
    auto* tv = qobject_cast<view::collections::TreeView*>(m_view);
...
void FluentTreeItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    if (!m_checkBoxVisible && isSelected && isEnabled && colors.accentDefault.isValid()) {
        ...
        const auto direction = activeMotion
                                   ...
                                   : view::collections::TreeView::IndicatorVerticalDirection::None;
        const auto hierarchyTransition = activeMotion
                                             ? tv->indicatorHierarchyTransition()
                                             : view::collections::TreeView::IndicatorHierarchyTransition::None;
        ...
        if (hierarchyTransition == view::collections::TreeView::IndicatorHierarchyTransition::Inward)
            indicatorX += remaining * 4.0;
        else if (hierarchyTransition == view::collections::TreeView::IndicatorHierarchyTransition::Outward)
            indicatorX -= remaining * 3.0;

        qreal indicatorY = bgRect.center().y() - indicatorH / 2.0;
        if (direction == view::collections::TreeView::IndicatorVerticalDirection::Down)
            indicatorY = settledY - remaining * 6.0;
        else if (direction == view::collections::TreeView::IndicatorVerticalDirection::Up)
        ...
        if (m_checkBoxVisible) {
            ...
            if (cbRect.contains(pos)) {
                ...
                const bool syncSelection = tv && selection
                    && (tv->selectionMode() == view::collections::TreeView::TreeSelectionMode::Multiple
                        || tv->selectionMode() == view::collections::TreeView::TreeSelectionMode::Extended);
