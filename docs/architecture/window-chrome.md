...
|---|---|---|---|
| Windows | `_win.cpp` | 自绘 caption（去掉 `WS_CAPTION`），自定义 hit-test | Win11 DWM Mica/Acrylic；Win10 legacy Acrylic 回退 |
...
`platformSupportsSystemBackdrop()` 决定 `fluentMicaBackdrop` 是否为真：Windows 11 / Windows 10 1803+ / macOS 为真（走透明 + 系统背景），其它为假（走纯色降级）。
