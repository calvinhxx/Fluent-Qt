...
`platformSupportsSystemBackdrop()` 决定 `fluentMicaBackdrop` 是否为真：Windows 11 22H2+ / macOS 为真（走透明 + 系统背景），Windows 10 和其它平台为假（走纯色降级）。Win10 legacy Acrylic 不再作为 window-level backdrop 使用，因为 Qt backing store 在 Win10/VM 上会把透明客户区渲染成黑色或黑边。
