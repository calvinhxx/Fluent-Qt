...
| WebAssembly browser canvas | `PaintedMaterial` | `Emulated / PaintedOpaque` | One browser-sized Qt desktop surface hosts bounded Fluent windows with an opaque active title surface, cached static window material, client-side caption buttons, manual move/resize, and maximize/restore. OS-native chrome, tray, system move/resize, and compositor APIs remain unavailable. |
