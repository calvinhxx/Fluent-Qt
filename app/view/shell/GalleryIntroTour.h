 ...
 * blocks clicks (modal); window move and resize chrome is disabled via Window::setChromeInteractive.
 * The card is a CoachMark (separate top-level) raised above, with a tail pointing at each target. Previous /
 ...
 * Window::setChromeInteractive 禁用。卡片是 CoachMark(独立顶层)浮于其上,尾巴指向每个目标。Previous/Next/Finish
