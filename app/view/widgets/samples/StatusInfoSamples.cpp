...
public:
    explicit StatusInfoSampleSurface(QWidget* parent = nullptr, int spacing = 12) : QWidget(parent)
    {
...
public:
    explicit SampleStatusPill(const QString& text, QWidget* parent = nullptr)
        : QWidget(parent), m_label(new Label(text, this))
    {
...
protected:
    void paintEvent(QPaintEvent*) override
    {
        ...
        painter.drawRoundedRect(rect().adjusted(0, 0, -1, -1), themeRadius().control,
                                themeRadius().control);
    }
...
QWidget* labeledColumn(QWidget* parent, const QString& labelText, QWidget* content,
...
InfoBadge* makeBadge(QWidget* parent, InfoBadge::InfoBadgeDisplayMode mode,
                     InfoBadge::InfoBadgeStatus status, int value = -1,
...
InfoBar* makeInfoBar(QWidget* parent, InfoBar::InfoBarSeverity severity, const QString& title,
                     const QString& message, bool singleLine = true)
{
...
QWidget* progressBarRow(QWidget* parent, const QString& labelText, ProgressBar* progressBar)
{
...
QWidget* progressRingColumn(QWidget* parent, const QString& labelText, ProgressRing* ring)
{
...
{
    return {
        makeSample(QStringLiteral("avatar-initials-sizes"), QStringLiteral("Initials and sizes"),
                   QStringLiteral("Avatar generates a compact initials fallback from the "
                                  "caller-provided name and offers four token-aligned sizes."),
                   ...
                   [](QWidget* parent) {
                       ...
                       const AvatarCase cases[] = {
                           {QStringLiteral("Ada Lovelace"), QStringLiteral("Small"),
                            Avatar::AvatarSize::Small},
                           {QStringLiteral("Grace Hopper"), QStringLiteral("Medium"),
                            Avatar::AvatarSize::Medium},
                           {QStringLiteral("Lin Chen"), QStringLiteral("Large"),
                            Avatar::AvatarSize::Large},
                           {QStringLiteral("Sam Rivera"), QStringLiteral("Extra large"),
                            Avatar::AvatarSize::ExtraLarge},
                       };
                       for (const AvatarCase& avatarCase : cases) {
                           auto* avatar = new Avatar(avatarCase.name, row);
                           avatar->setAvatarSize(avatarCase.size);
                           boxLayout(row)->addWidget(
                               labeledColumn(row, avatarCase.caption, avatar));
                       }
                       boxLayout(surface)->addWidget(row, 0, Qt::AlignLeft);
                       return surface;
                   }),
        makeSample(
            QStringLiteral("avatar-image-presence"), QStringLiteral("Image, shape, and presence"),
            QStringLiteral("A high-DPI pixmap replaces initials, while the composed InfoBadge "
                           "communicates presence without adding text."),
            ...
            [](QWidget* parent) {
                ...
                auto* profile = new Avatar(QStringLiteral("Product account"), row);
                profile->setAvatarSize(Avatar::AvatarSize::ExtraLarge);
                profile->setImage(QPixmap(QStringLiteral(":/app/assets/app-icon.png")));
                profile->setPresence(Avatar::PresenceStatus::Available);
                boxLayout(row)->addWidget(labeledColumn(row, QStringLiteral("Available"), profile));

                auto* away = new Avatar(QStringLiteral("Alex Morgan"), row);
                away->setAvatarSize(Avatar::AvatarSize::Large);
                away->setShape(Avatar::AvatarShape::Square);
                away->setPresence(Avatar::PresenceStatus::Away);
                boxLayout(row)->addWidget(labeledColumn(row, QStringLiteral("Away"), away));

                auto* busy = new Avatar(QStringLiteral("Jordan Lee"), row);
                busy->setAvatarSize(Avatar::AvatarSize::Large);
                busy->setPresence(Avatar::PresenceStatus::Busy);
                boxLayout(row)->addWidget(labeledColumn(row, QStringLiteral("Busy"), busy));

                auto* offline = new Avatar(QStringLiteral("Taylor Reed"), row);
                offline->setAvatarSize(Avatar::AvatarSize::Large);
                offline->setPresence(Avatar::PresenceStatus::Offline);
                boxLayout(row)->addWidget(labeledColumn(row, QStringLiteral("Offline"), offline));
...
public:
    HoverToolTipController(QWidget* target, ToolTip* toolTip,
                           ToolTipPlacement placement = ToolTipPlacement::Above)
        : QObject(target), m_target(target), m_toolTip(toolTip), m_placement(placement)
    {
    ...
    ~HoverToolTipController() override { delete m_toolTip; }
        ...
        if (watched == m_target) {
            if (event->type() == QEvent::Enter) {
                ...
                const QPoint localAnchor =
                    m_placement == ToolTipPlacement::Right
                        ? QPoint(m_target->width() + 8,
                                 m_target->height() / 2 - tipSize.height() / 2)
                        : QPoint(m_target->width() / 2 - tipSize.width() / 2,
                                 -tipSize.height() - 4);
...
QVector<GallerySample> infoBadgeSamples()
{
    return {
        makeSample(
            QStringLiteral("info-badge-display-modes"), QStringLiteral("Display modes"),
            ...
            [](QWidget* parent) {
                ...
                boxLayout(row)->addWidget(
                    labeledColumn(row, QStringLiteral("Auto icon"), iconBadge));
                boxLayout(row)->addWidget(
                    labeledColumn(row, QStringLiteral("Auto value"), valueBadge));
                boxLayout(row)->addWidget(
                    labeledColumn(row, QStringLiteral("Explicit dot"), explicitDot));
                return surface;
            }),
        makeSample(QStringLiteral("info-badge-status-colors"), QStringLiteral("Status colors"),
                   QStringLiteral(
                       "Status selects the semantic color used by dot, icon, and value badges."),
                   QStringLiteral(
                       "auto* successBadge = new InfoBadge(this);\n"
                   ...
                   [](QWidget* parent) {
                       ...
                       for (const StatusBadge& item : statuses) {
                           auto* badge = makeBadge(row, InfoBadge::InfoBadgeDisplayMode::Value,
                                                   item.status, 5);
        ...
        makeSample(QStringLiteral("info-badge-custom-metrics"),
                   ...
                   QStringLiteral("Badge sizing, padding, and color overrides let a badge sit on "
        ...
        makeSample(QStringLiteral("info-badge-accessibility"),
                   ...
                   QStringLiteral(
                       "A value badge remains a non-focusable child of its accessible parent and "
                       "reports value or visibility changes without duplicating business text."),
                   QStringLiteral(
                   ...
                   [](QWidget* parent) {
                       QWidget* surface = sampleSurface(parent);
                       auto* inbox = new Button(QStringLiteral("Inbox"), surface);
                       inbox->setAccessibleName(QStringLiteral("Inbox"));
                       inbox->setFixedSize(164, 42);

                       auto* badge = new InfoBadge(inbox);
                       badge->setObjectName(QStringLiteral("galleryInfoBadgeAccessibleValue"));
                       badge->setDisplayMode(InfoBadge::InfoBadgeDisplayMode::Value);
                       badge->setAccessibleName(QStringLiteral("Unread messages"));
                       badge->setValue(3);
                       badge->resize(badge->sizeHint());
                       badge->move(inbox->width() - badge->width() - 6, 4);

                       QWidget* row = horizontalGroup(surface, 8);
                       auto* increment = sampleButton(row, QStringLiteral("Increment"));
                       increment->setObjectName(
                           QStringLiteral("galleryInfoBadgeAccessibleIncrement"));
                       auto* toggle = sampleButton(row, QStringLiteral("Toggle badge"));
                       toggle->setObjectName(QStringLiteral("galleryInfoBadgeAccessibleToggle"));
                       auto* status = makeStatusLabel(surface, QStringLiteral("Unread value: 3"));
                       status->setObjectName(QStringLiteral("galleryInfoBadgeAccessibleStatus"));

                       QObject::connect(increment, &Button::clicked, badge, [badge, status]() {
                           badge->setValue(badge->value() + 1);
                           status->setText(QStringLiteral("Unread value: %1").arg(badge->value()));
                       });
                       QObject::connect(toggle, &Button::clicked, badge, [badge, status]() {
                           const bool showBadge = badge->isHidden();
                           badge->setVisible(showBadge);
                           status->setText(showBadge ? QStringLiteral("Badge visible")
                                                     : QStringLiteral("Badge hidden"));
                       });

                       boxLayout(row)->addWidget(increment);
                       boxLayout(row)->addWidget(toggle);
                       boxLayout(surface)->addWidget(inbox, 0, Qt::AlignLeft);
                       boxLayout(surface)->addWidget(row);
...
QVector<GallerySample> infoBarSamples()
{
    return {
        makeSample(
            QStringLiteral("info-bar-severities"), QStringLiteral("Severity"),
            QStringLiteral(
                "Severity changes the leading icon and background treatment for status messages."),
            ...
            [](QWidget* parent) {
                ...
                const QVector<InfoBar*> bars{
                    makeInfoBar(surface, InfoBar::Informational, QStringLiteral("Update available"),
                                QStringLiteral("Version 3.2 is ready.")),
                    makeInfoBar(surface, InfoBar::Success, QStringLiteral("Saved"),
                                QStringLiteral("All changes were saved.")),
                    makeInfoBar(surface, InfoBar::Warning, QStringLiteral("Storage almost full"),
                                QStringLiteral("Clear space before syncing.")),
                    makeInfoBar(surface, InfoBar::Error, QStringLiteral("Upload failed"),
        ...
        makeSample(QStringLiteral("info-bar-action-layout"), QStringLiteral("Multi-line action"),
                   QStringLiteral(
                       "Long messages switch to multi-line layout and can host an action widget."),
                   QStringLiteral("auto* retryButton = new Button(\"Retry\", this);\n"
                                  ...
                                  "infoBar->setMessage(\"Some files need attention before the next "
                                  "sync can finish.\");\n"
                   ...
                   [](QWidget* parent) {
                       ...
                       auto* infoBar = makeInfoBar(
                           surface, InfoBar::Warning, QStringLiteral("Sync paused"),
                           QStringLiteral(
        ...
        makeSample(
            QStringLiteral("info-bar-open-close"), QStringLiteral("Close notification"),
            QStringLiteral("closed() hides the InfoBar; a command can reopen it when the message "
                           "is needed again."),
            ...
            [](QWidget* parent) {
                ...
                openButton->setIconGlyph(Typography::Icons::Refresh,
                ...
                auto* infoBar =
                    makeInfoBar(surface, InfoBar::Informational, QStringLiteral("Draft saved"),
                ...
                QObject::connect(openButton, &Button::clicked, openButton,
                                 [openButton]() { openButton->setEnabled(false); });
...
QVector<GallerySample> progressBarSamples()
{
    return {
        makeSample(
            QStringLiteral("progress-bar-determinate-value"), QStringLiteral("Determinate value"),
            QStringLiteral(
                "A determinate ProgressBar exposes range, value, ratio, and progress text."),
            QStringLiteral("auto* progressBar = new ProgressBar(this);\n"
                           ...
                           "progressBox->setSpinButtonPlacementMode(NumberBox::"
                           "SpinButtonPlacementMode::Inline);\n"
                           "\n"
                           "auto* status = new Label(\"Progress: 44%\", this);\n"
                           "status->setFixedWidth(124);\n"
                           "\n"
                           "connect(progressBox, &NumberBox::valueChanged, progressBar, "
                           "[progressBar, status](double value) {\n"
                           "    progressBar->setValue(std::isfinite(value) ? value : 0.0);\n"
                           "    status->setText(QStringLiteral(\"Progress: "
                           "%1%\").arg(progressBar->progressText()));\n"
                           "});\n"
            ...
            [](QWidget* parent) {
                ...
                auto* status = makeValueStatusLabel(surface, QStringLiteral("Progress: 44%"));
                ...
                QObject::connect(
                    progressBox, &NumberBox::valueChanged, progressBar,
                    [progressBar, status](double value) {
                        progressBar->setValue(std::isfinite(value) ? value : 0.0);
                        status->setText(
                            QStringLiteral("Progress: %1%").arg(progressBar->progressText()));
                    });
                progressBox->setValue(44);
        ...
        makeSample(QStringLiteral("progress-bar-states"),
                   ...
                   QStringLiteral("Indeterminate bars animate while running; paused and error "
                                  "states stop motion and change color."),
                   ...
                   [](QWidget* parent) {
                       ...
                       boxLayout(surface)->addWidget(
                           progressBarRow(surface, QStringLiteral("Running"), running));
                       boxLayout(surface)->addWidget(
                           progressBarRow(surface, QStringLiteral("Paused"), paused));
                       boxLayout(surface)->addWidget(
                           progressBarRow(surface, QStringLiteral("Error"), error));
                       return surface;
                   }),
        makeSample(QStringLiteral("progress-bar-metrics"), QStringLiteral("Rail and thickness"),
                   QStringLiteral("The rail, width, and track thickness define the geometry "
                                  "without changing value semantics."),
                   ...
                   [](QWidget* parent) {
                       ...
                       boxLayout(surface)->addWidget(
                           progressBarRow(surface, QStringLiteral("6 px track"), thickBar));
                       boxLayout(surface)->addWidget(
                           progressBarRow(surface, QStringLiteral("No rail"), noRailBar));
                       return surface;
                   })};
}
...
QVector<GallerySample> progressRingSamples()
{
    return {
        makeSample(
            QStringLiteral("progress-ring-indeterminate-sizes"),
            QStringLiteral("Active indeterminate sizes"),
            QStringLiteral(
                "An active indeterminate ProgressRing animates in small, medium, and large sizes."),
            ...
            [](QWidget* parent) {
                ...
                boxLayout(row)->addWidget(
                    progressRingColumn(row, QStringLiteral("Medium"), medium));
                boxLayout(row)->addWidget(progressRingColumn(row, QStringLiteral("Large"), large));
                boxLayout(surface)->addWidget(row);
                return surface;
            }),
        makeSample(
            QStringLiteral("progress-ring-determinate-value"), QStringLiteral("Determinate value"),
            QStringLiteral("Determinate rings use range, value, and an optional background track."),
            QStringLiteral(
                "auto* ring = new ProgressRing(this);\n"
                ...
                "progressBox->setLargeChange(10);\n"
                "progressBox->setDisplayPrecision(0);\n"
                "progressBox->setSpinButtonPlacementMode(NumberBox::SpinButtonPlacementMode::"
                "Inline);\n"
                "\n"
            ...
            [](QWidget* parent) {
                ...
                QObject::connect(
                    progressBox, &NumberBox::valueChanged, ring, [ring, status](double value) {
                        const int progress = std::isfinite(value) ? static_cast<int>(value) : 0;
                        ring->setValue(progress);
                        status->setText(QStringLiteral("Value: %1%").arg(progress));
                    });
        ...
        makeSample(
            QStringLiteral("progress-ring-status"), QStringLiteral("Status states"),
            QStringLiteral(
                "Paused and error statuses reuse determinate geometry with semantic colors."),
            ...
            [](QWidget* parent) {
                ...
                boxLayout(row)->addWidget(
                    progressRingColumn(row, QStringLiteral("Running"), running));
                boxLayout(row)->addWidget(
                    progressRingColumn(row, QStringLiteral("Paused"), paused));
...
QVector<GallerySample> shimmerSamples()
{
    return {
        makeSample(
            QStringLiteral("shimmer-templates"), QStringLiteral("Built-in templates"),
            ...
            [](QWidget* parent) {
                ...
                boxLayout(row)->addWidget(
                    labeledColumn(row, QStringLiteral("AvatarTextRow"), avatarRow));
                boxLayout(row)->addWidget(labeledColumn(row, QStringLiteral("TextBlock"), text));
                boxLayout(surface)->addWidget(row);
                return surface;
            }),
        makeSample(QStringLiteral("shimmer-custom-elements"), QStringLiteral("Custom elements"),
                   QStringLiteral(
                       "Custom skeleton elements let a shimmer match the loading content shape."),
        ...
        makeSample(
            QStringLiteral("shimmer-static-phase"), QStringLiteral("Static phase"),
            QStringLiteral("Animation can be disabled while keeping a deterministic shimmer "
                           "highlight visible."),
            QStringLiteral("auto* shimmer = new Shimmer(this);\n"
    ...
    return {
        makeSample(
            ...
            [](QWidget* parent) {
                ...
                toast->setObjectName(QStringLiteral("galleryToastSeveritySample"));
                toast->setDuration(2200);
                QObject::connect(surface, &QObject::destroyed, toast, &QObject::deleteLater);
                QWidget* row = horizontalGroup(surface, 8);
                ...
                const SeverityAction actions[] = {
                    {QStringLiteral("Info"), QStringLiteral("Draft saved locally"),
                     Toast::Informational},
                    {QStringLiteral("Success"), QStringLiteral("Changes published"),
                     Toast::Success},
                    {QStringLiteral("Warning"), QStringLiteral("Connection is unstable"),
                     Toast::Warning},
                    {QStringLiteral("Error"), QStringLiteral("Upload could not finish"),
                     Toast::Error},
                };
                for (const SeverityAction& action : actions) {
                    auto* button = sampleButton(row, action.title);
                    QObject::connect(button, &Button::clicked, button, [button, toast, action]() {
                ...
                boxLayout(surface)->addWidget(makeStatusLabel(
                    surface, QStringLiteral("The same toast instance updates for each severity.")));
                return surface;
            }),
        makeSample(QStringLiteral("toast-title-placement"), QStringLiteral("Title and placement"),
                   QStringLiteral("Add a concise title when the message needs context, and anchor "
                                  "the toast to any top or bottom edge."),
                   QStringLiteral("auto* toast = new Toast(this);\n"
                   ...
                   [](QWidget* parent) {
                       ...
                       QWidget* row = horizontalGroup(surface, 8);
                       auto* toast = new Toast(surface);
                       toast->setObjectName(QStringLiteral("galleryToastPlacementSample"));
                       toast->setTitle(QStringLiteral("Sync complete"));
                       toast->setMessage(QStringLiteral("12 files are now available offline."));
                       toast->setSeverity(Toast::Success);
                       toast->setDuration(2600);
                       QObject::connect(surface, &QObject::destroyed, toast, &QObject::deleteLater);
        ...
        makeSample(
            QStringLiteral("toast-stacking"), QStringLiteral("Stacking"),
            QStringLiteral("Managed toasts stack per placement up to Toast::maximumVisible(), "
                           "shifting away from the chosen edge in show order."),
            ...
            [](QWidget* parent) {
                QWidget* surface = sampleSurface(parent);
                QWidget* row = horizontalGroup(surface, 8);
                auto* stackTop = sampleButton(row, QStringLiteral("Stack at top"));
                auto* stackEnd = sampleButton(row, QStringLiteral("Stack at top end"));
                QObject::connect(stackTop, &Button::clicked, stackTop, [stackTop]() {
                    static int counter = 0;
                    ++counter;
                    Toast::showToast(stackTop, QStringLiteral("Top notice %1").arg(counter),
                                     Toast::Informational);
                });
                QObject::connect(stackEnd, &Button::clicked, stackEnd, [stackEnd]() {
                    static int counter = 0;
                    ++counter;
                    Toast::showToast(stackEnd, QStringLiteral("Corner notice %1").arg(counter),
                                     Toast::Warning, 2200, Toast::TopEnd);
                });
                ...
                boxLayout(surface)->addWidget(makeStatusLabel(
                    surface,
                    QStringLiteral("Default maximumVisible is 3; older toasts dismiss first.")));
                return surface;
            }),
        makeSample(
            QStringLiteral("toast-action-lifecycle"),
            QStringLiteral("Action, hover pause, and dismissal reason"),
            QStringLiteral("An optional borrowed QAction makes the toast interactive; hover can "
                           "pause its timeout, and every close path reports a stable reason."),
            QStringLiteral(
            ...
            [](QWidget* parent) {
                QWidget* surface = sampleSurface(parent);
                auto* retry = new QAction(QStringLiteral("Retry"), surface);
                auto* toast = new Toast(surface);
                toast->setObjectName(QStringLiteral("galleryToastLifecycleSample"));
                toast->setAction(retry);
                toast->setPauseOnHoverEnabled(true);
                toast->setDuration(5000);
                QObject::connect(surface, &QObject::destroyed, toast, &QObject::deleteLater);

                auto* showToast = sampleButton(surface, QStringLiteral("Show actionable toast"));
                showToast->setObjectName(QStringLiteral("galleryToastLifecycleTrigger"));
                auto* status = makeStatusLabel(surface, QStringLiteral("Ready"));
                status->setObjectName(QStringLiteral("galleryToastLifecycleStatus"));

                QObject::connect(showToast, &Button::clicked, toast, [showToast, toast, status]() {
                    toast->setMessage(QStringLiteral("Upload failed. Retry when ready."));
                    toast->setSeverity(Toast::Error);
                    toast->present(showToast);
                    status->setText(QStringLiteral("Toast open; hover pauses timeout"));
                });
                QObject::connect(retry, &QAction::triggered, status, [status]() {
                    status->setText(QStringLiteral("Retry requested"));
                });
                QObject::connect(
                    toast, &Toast::dismissedWithReason, status,
                    [status](Toast::DismissReason reason) {
                        const QMetaEnum meta = QMetaEnum::fromType<Toast::DismissReason>();
                        status->setText(QStringLiteral("Dismissed: %1")
                                            .arg(QString::fromLatin1(meta.valueToKey(reason))));
                    });

                boxLayout(surface)->addWidget(showToast, 0, Qt::AlignLeft);
                boxLayout(surface)->addWidget(status);
                return surface;
            }),
        makeSample(
            QStringLiteral("toast-update-key"), QStringLiteral("In-place managed updates"),
            QStringLiteral("A non-empty update key refreshes the matching toast within one host "
                           "and placement without adding another stack entry."),
            ...
            [](QWidget* parent) {
                QWidget* surface = sampleSurface(parent);
                auto* advance = sampleButton(surface, QStringLiteral("Advance upload"));
                advance->setObjectName(QStringLiteral("galleryToastUpdateTrigger"));
                advance->setProperty("progress", 0);
                auto* status = makeStatusLabel(surface, QStringLiteral("Progress: 0%"));
                status->setObjectName(QStringLiteral("galleryToastUpdateStatus"));

                QObject::connect(advance, &Button::clicked, advance, [advance, status]() {
                    const int current = advance->property("progress").toInt();
                    const int progress = current >= 100 ? 25 : current + 25;
                    advance->setProperty("progress", progress);
                    Toast::showOrUpdateToast(
                        advance, QStringLiteral("upload"),
                        progress == 100 ? QStringLiteral("Upload complete")
                                        : QStringLiteral("Uploading: %1%").arg(progress),
                        progress == 100 ? Toast::Success : Toast::Informational, 5000,
                        Toast::TopEnd);
                    status->setText(QStringLiteral("Progress: %1%").arg(progress));
                });

                boxLayout(surface)->addWidget(advance, 0, Qt::AlignLeft);
                boxLayout(surface)->addWidget(status);
                return surface;
            })};
}
...
QVector<GallerySample> toolTipSamples()
{
    return {
        makeSample(
            QStringLiteral("tooltip-hover"), QStringLiteral("Hover target"),
        ...
        makeSample(
            QStringLiteral("tooltip-margins-font"), QStringLiteral("Margins and font"),
        ...
        makeSample(QStringLiteral("tooltip-animation"), QStringLiteral("Animation toggle"),
                   QStringLiteral(
                       "Animation can be disabled for deterministic or instant tooltip behavior."),
