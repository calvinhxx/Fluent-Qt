...
static_assert(static_cast<int>(fluent::binding::Theme::Light) ==
              static_cast<int>(fluent::FluentElement::Light));
static_assert(static_cast<int>(fluent::binding::Theme::Dark) ==
              static_cast<int>(fluent::FluentElement::Dark));
static_assert(static_cast<int>(fluent::binding::Theme::HighContrast) ==
              static_cast<int>(fluent::FluentElement::HighContrast));
static_assert(static_cast<int>(fluent::binding::MotionMode::Full) ==
              static_cast<int>(fluent::MotionPolicy::Mode::Full));
static_assert(static_cast<int>(fluent::binding::MotionMode::Reduced) ==
              static_cast<int>(fluent::MotionPolicy::Mode::Reduced));
static_assert(static_cast<int>(fluent::binding::MotionMode::Disabled) ==
              static_cast<int>(fluent::MotionPolicy::Mode::Disabled));
static_assert(static_cast<int>(fluent::binding::MotionKind::Transition) ==
              static_cast<int>(fluent::MotionPolicy::Kind::Transition));
static_assert(static_cast<int>(fluent::binding::MotionKind::Continuous) ==
              static_cast<int>(fluent::MotionPolicy::Kind::Continuous));
static_assert(static_cast<int>(fluent::binding::SelectionMode::None) ==
              static_cast<int>(fluent::collections::SelectionMode::None));
static_assert(static_cast<int>(fluent::binding::SelectionMode::Single) ==
              static_cast<int>(fluent::collections::SelectionMode::Single));
static_assert(static_cast<int>(fluent::binding::SelectionMode::Multiple) ==
              static_cast<int>(fluent::collections::SelectionMode::Multiple));
static_assert(static_cast<int>(fluent::binding::SelectionMode::Extended) ==
              static_cast<int>(fluent::collections::SelectionMode::Extended));
static_assert(static_cast<int>(fluent::binding::BindingMode::OneWay) ==
              static_cast<int>(fluent::PropertyBinder::OneWay));
static_assert(static_cast<int>(fluent::binding::BindingMode::TwoWay) ==
...
{
    QVariantMap values;
#define FLUENTQT_COLOR_TOKEN(name) values.insert(QStringLiteral(#name), colors.name)
...
QVariantMap spacingTokens(const fluent::FluentElement::Spacing& spacing)
{
    ...
    controlHeight.insert(QStringLiteral("standard"), spacing.controlHeight.standard);
...
const fluent::FluentElement* nearestFluentElement(const QWidget* widget)
{
    for (const QWidget* node = widget; node; node = node->parentWidget()) {
        if (const auto* element = dynamic_cast<const fluent::FluentElement*>(node)) {
            return element;
        }
    }
    return nullptr;
...
class CurrentThemeTokenProbe final : public fluent::FluentElement {
public:
    CurrentThemeTokenProbe() = default;
};
...
QVariantMap themeTokens(const fluent::FluentElement& element)
{
    ...
    QVariantMap easings;
    easings.insert(QStringLiteral("standard"), QVariant::fromValue(animation.standard));
    easings.insert(QStringLiteral("accelerate"), QVariant::fromValue(animation.accelerate));
    easings.insert(QStringLiteral("decelerate"), QVariant::fromValue(animation.decelerate));
    easings.insert(QStringLiteral("entrance"), QVariant::fromValue(animation.entrance));
    easings.insert(QStringLiteral("exit"), QVariant::fromValue(animation.exit));
    ...
    acrylicValues.insert(QStringLiteral("luminosityOpacity"), acrylic.luminosityOpacity);
    ...
    elevationValues.insert(QStringLiteral("none"), shadowValues(Elevation::None));
    elevationValues.insert(QStringLiteral("low"), shadowValues(Elevation::Low));
    elevationValues.insert(QStringLiteral("medium"), shadowValues(Elevation::Medium));
    elevationValues.insert(QStringLiteral("high"), shadowValues(Elevation::High));
    elevationValues.insert(QStringLiteral("veryHigh"), shadowValues(Elevation::VeryHigh));

    QVariantMap breakpointValues;
    breakpointValues.insert(QStringLiteral("small"),
                            element.themeBreakpoint(Breakpoints::Breakpoint::Small));
    breakpointValues.insert(QStringLiteral("medium"),
                            element.themeBreakpoint(Breakpoints::Breakpoint::Medium));
    breakpointValues.insert(QStringLiteral("large"),
                            element.themeBreakpoint(Breakpoints::Breakpoint::Large));

    QVariantMap backdropValues;
    backdropValues.insert(QStringLiteral("active"), element.themeBackdrop(true));
    backdropValues.insert(QStringLiteral("inactive"), element.themeBackdrop(false));

    QVariantMap values;
    values.insert(QStringLiteral("theme"), static_cast<int>(element.effectiveTheme()));
    values.insert(QStringLiteral("colors"), colorTokens(element.themeColors()));
    values.insert(QStringLiteral("radius"), radiusValues);
    values.insert(QStringLiteral("spacing"), spacingTokens(element.themeSpacing()));
...
fluent::binding::AnchorSpec::AnchorSpec() : d_ptr(new AnchorSpecPrivate) {}

fluent::binding::AnchorSpec::AnchorSpec(const AnchorSpec& other)
    : d_ptr(new AnchorSpecPrivate(*other.d_ptr))
{}

fluent::binding::AnchorSpec& fluent::binding::AnchorSpec::operator=(const AnchorSpec& other)
{
    if (this != &other)
        *d_ptr = *other.d_ptr;
    return *this;
...
void fluent::binding::AnchorSpec::setAnchor(AnchorEdge sourceEdge, QWidget* target,
                                            AnchorEdge targetEdge, int offset)
{
    const fluent::AnchorLayout::Anchor anchor(target, toAnchorEdge(targetEdge), offset);
    switch (sourceEdge) {
...
void fluent::binding::AnchorLayout::addWidget(QWidget* widget, const AnchorSpec& anchors)
{
    if (m_layout)
...
fluent::binding::StateGroup::~StateGroup()
{
    delete d_ptr;
}

bool fluent::binding::StateGroup::addStateChange(const QString& name, QObject* target,
                                                 const QString& propertyName, const QVariant& value)
{
    ...
    if (propertyIndex < 0 && !target->dynamicPropertyNames().contains(property)) {
        return false;
    }
    if (propertyIndex >= 0 && !target->metaObject()->property(propertyIndex).isWritable()) {
        return false;
    }
...
void fluent::binding::StateGroup::clearStateDefinition(const QString& name)
{
    if (name.isEmpty())
        return;
    fluent::QMLState& state = d_ptr->states[name];
...
fluent::binding::FluentWidget::FluentWidget(QWidget* parent) : QWidget(parent) {}
...
QFont fluent::binding::FluentWidget::themeFontForBinding(Typography::FontRole role) const
{
    return themeFont(role).toQFont();
}

fluent::binding::Theme fluent::binding::FluentWidget::effectiveThemeForBinding() const
{
    return static_cast<fluent::binding::Theme>(static_cast<int>(effectiveTheme()));
}

void fluent::binding::FluentWidget::onThemeUpdated()
{
    update();
}

fluent::binding::ScrollViewZoomAwareWidget::ScrollViewZoomAwareWidget(QWidget* parent)
    : QWidget(parent)
{}

fluent::binding::ScrollViewZoomAwareWidget::~ScrollViewZoomAwareWidget() = default;

QSizeF fluent::binding::ScrollViewZoomAwareWidget::scrollViewUnscaledSize() const
{
    return QSizeF(size()) / m_zoomFactor;
}

void fluent::binding::ScrollViewZoomAwareWidget::setScrollViewZoomFactor(qreal factor)
{
    if (factor <= 0.0)
        return;
...
{
    return fluent::initializeResources();
}
...
{
    return fluent::FluentElement::themeUsesDarkAppearance(
        static_cast<fluent::FluentElement::Theme>(static_cast<int>(theme)));
}

void setMotionMode(fluent::binding::MotionMode mode)
{
    fluent::MotionPolicy::instance().setMode(
        static_cast<fluent::MotionPolicy::Mode>(static_cast<int>(mode)));
}

fluent::binding::MotionMode currentMotionMode()
{
    return static_cast<fluent::binding::MotionMode>(
        static_cast<int>(fluent::MotionPolicy::instance().mode()));
}

bool shouldAnimateMotion(bool localAnimationEnabled, fluent::binding::MotionKind kind)
{
    return fluent::MotionPolicy::instance().shouldAnimate(
        localAnimationEnabled, static_cast<fluent::MotionPolicy::Kind>(static_cast<int>(kind)));
}

int resolvedMotionDuration(int fullDurationMs, bool localAnimationEnabled)
{
    return fluent::MotionPolicy::instance().resolvedDuration(fullDurationMs, localAnimationEnabled);
}
...
qreal fontScale()
{
    return fluent::ThemeRegistry::instance().fontScale();
}
...
QVariantMap themeTokensForWidgetForBinding(const QWidget* widget)
{
    ...
    if (element)
        return themeTokens(*element);

    CurrentThemeTokenProbe probe;
    return themeTokens(probe);
}
                                    ...
                                    bool checkNestedScrolling, bool checkLayoutGrid)
{
    ...
    options.minimumHitArea = QSize(qMax(1, minimumHitWidth), qMax(1, minimumHitHeight));
...
{
    if (!source || !target || sourceProperty.isEmpty() || targetProperty.isEmpty()) {
        return false;
    }
    ...
    const QMetaProperty sourceMeta = source->metaObject()->property(sourceIndex);
    const QMetaProperty targetMeta = target->metaObject()->property(targetIndex);
    ...
    fluent::PropertyBinder::bind(
        source, sourceName.constData(), target, targetName.constData(),
        static_cast<fluent::PropertyBinder::Direction>(static_cast<int>(mode)));
    return true;
}

fluent::binding::SelectionMode flowViewSelectionMode(const fluent::collections::FlowView* view)
{
    return static_cast<fluent::binding::SelectionMode>(static_cast<int>(view->selectionMode()));
}

void setFlowViewSelectionMode(fluent::collections::FlowView* view,
                              fluent::binding::SelectionMode mode)
{
    view->setSelectionMode(static_cast<fluent::collections::SelectionMode>(static_cast<int>(mode)));
}
...
void setListViewSectionKeys(fluent::collections::ListView* view, const QStringList& keys)
{
    ...
    view->setSectionKeyFunction(
        [keys](int row) { return row >= 0 && row < keys.size() ? keys.at(row) : QString(); });
}
                                      ...
                                      const QString& elementsJson)
{
    ...
    const QJsonDocument document = QJsonDocument::fromJson(elementsJson.toUtf8(), &parseError);
    ...
    for (const QJsonValue& value : values) {
        ...
        const auto shape =
            static_cast<fluent::status_info::ShimmerPainter::Shape>(qBound(0, shapeValue, 3));
                    ...
                    fluent::status_info::Toast::Placement placement, const QMargins& margins)
{
    if (!host || !anchor || anchor->window() != host)
        return nullptr;
    return fluent::status_info::Toast::showToast(anchor, message, severity, durationMs, placement,
                                                 margins);
}
                            ...
                            const QMargins& margins)
{
    if (!host || !anchor || anchor->window() != host)
        return nullptr;
    return fluent::status_info::Toast::showOrUpdateToast(anchor, updateKey, message, severity,
                                                         durationMs, placement, margins);
}

int themeRevision()
{
    return fluent::ThemeRegistry::instance().revision();
}
...
QVariantMap bindingBuildInfo()
{
    ...
    info.insert(QStringLiteral("pyside6_version"), QStringLiteral(FLUENT_QT_PYSIDE6_VERSION));
    info.insert(QStringLiteral("shiboken6_version"), QStringLiteral(FLUENT_QT_SHIBOKEN6_VERSION));
    info.insert(QStringLiteral("shiboken6_generator_version"),
                QStringLiteral(FLUENT_QT_SHIBOKEN6_GENERATOR_VERSION));
    info.insert(QStringLiteral("qt_compile_version"), QStringLiteral(QT_VERSION_STR));
    info.insert(QStringLiteral("qt_runtime_version"), QString::fromLatin1(qVersion()));
