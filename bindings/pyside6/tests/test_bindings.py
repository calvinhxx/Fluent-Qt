...
class FluentQtBindingTest(unittest.TestCase):
    ...
    def test_splash_screen_public_api_and_reuse(self):
        previous = fluentqt.current_motion_mode()
        self.addCleanup(fluentqt.set_motion_mode, previous)
        fluentqt.set_motion_mode(fluentqt.MotionMode.Disabled)
        host = QWidget()
        host.resize(220, 180)
        splash = fluentqt.SplashScreen(host)
        self.assertIs(status_info.SplashScreen, fluentqt.SplashScreen)
        self.assertIs(splash.parentWidget(), host)
        self.assertTrue(splash.isIndeterminate())
        ...
        self.assertEqual(splash.title(), "Workspace")
        self.assertEqual(splash.subtitle(), "Ready for your next idea")
        target = QWidget(host)
        splash.setTransitionTarget(target)
        self.assertIs(splash.transitionTarget(), target)
        splash.setTransitionTarget(None)
        self.assertIsNone(splash.transitionTarget())
        splash.setPresentation(fluentqt.SplashScreen.Presentation.Simple)
        self.assertEqual(splash.presentation(), fluentqt.SplashScreen.Presentation.Simple)
        splash.setPresentation(fluentqt.SplashScreen.Presentation.Branded)
        splash.setText("Loading workspace")
        splash.setIconSize(QSize(64, 64))
        values = []
        splash.progressChanged.connect(values.append)
        splash.setProgress(3, 5)
        splash.setProgress(60, 100)
        self.assertEqual(values, [60])
        self.assertEqual(splash.progress(), 60)
        self.assertFalse(splash.isIndeterminate())
        self.assertEqual(splash.text(), "Loading workspace")
        self.assertEqual(splash.iconSize(), QSize(64, 64))
        finished = []
        splash.dismissed.connect(lambda: finished.append(True))
        try:
            host.show()
            splash.show()
            QApplication.processEvents()
            self.assertEqual(splash.geometry(), host.rect())
            splash.dismiss()
            self.assertTrue(shiboken6.isValid(splash))
            ...
            self.assertTrue(splash.isIndeterminate())
            replaced = QSignalSpy(splash.replaced)
            splash.show()
            replacement = fluentqt.SplashScreen(host)
            replacement.show()
            self.assertTrue(splash.isHidden())
    ...
    def test_public_types_and_build_versions(self):
        ...
        self.assertIs(foundation.MotionKind, fluentqt.MotionKind)
        self.assertIs(foundation.MotionMode, fluentqt.MotionMode)
        self.assertIs(foundation.MotionPolicy, fluentqt.MotionPolicy)
        self.assertIs(foundation.motion_policy(), fluentqt.motion_policy())
